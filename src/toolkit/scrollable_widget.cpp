...
bool ScrollableWidget::handle_mouse(MouseEvent const &event) {
    return handle_scrollbar_mouse(event);
}

bool ScrollableWidget::handle_scrollbar_mouse(MouseEvent const &event) {
    if (vscroll_ && vscroll_->is_visible()) {
        if (vscroll_->rect().contains(event.position) ||
            ((event.type == MouseEvent::Type::Drag || event.type == MouseEvent::Type::Release) &&
             vscroll_->is_dragging())) {
            return Widget::dispatch_mouse_event(vscroll_.get(), event);
        }
    }
    if (hscroll_ && hscroll_->is_visible()) {
        if (hscroll_->rect().contains(event.position) ||
            ((event.type == MouseEvent::Type::Drag || event.type == MouseEvent::Type::Release) &&
             hscroll_->is_dragging())) {
