...
void ScatterPlot::add_series(ScatterSeries series) { series_.push_back(std::move(series)); }

void ScatterPlot::clear_series() {
    series_.clear();
    hover_.reset();
    interaction_.reset_zoom();
}

ScatterPlot::PlotArea ScatterPlot::compute_plot_area() const {
    ...
    if (!first) {
        auto x_range = pa.data_x_max - pa.data_x_min;
        if (x_range < chart_defaults::kMinDataRange) {
            x_range = 1.0f;
        }
        auto x_pad = x_range * 0.08f;
        pa.data_x_min -= x_pad;
        pa.data_x_max += x_pad;
...
float ScatterPlot::to_screen_x(PlotArea const &pa, float data_x) const {
    auto t = (data_x - pa.data_x_min) / (pa.data_x_max - pa.data_x_min);
    return pa.x + t * pa.w;
}

float ScatterPlot::to_screen_y(PlotArea const &pa, float data_y) const {
    auto t = (data_y - pa.data_y_min) / (pa.data_y_max - pa.data_y_min);
    return pa.y + pa.h - t * pa.h;
}

void ScatterPlot::compute_nice_ticks(float min_val, float max_val, int target_count,
...
void ScatterPlot::paint(Painter &painter) {
    ...
    compute_nice_ticks(pa.data_x_min, pa.data_x_max, 6, x_ticks);
    for (auto xv : x_ticks) {
    ...
    if (pa.data_x_min < 0 && pa.data_x_max > 0) {
        auto zx = to_screen_x(pa, 0);
        painter.draw_line({zx, pa.y}, {zx, pa.y + pa.h}, axis_color, 0.5f);
    }
    if (pa.data_y_min < 0 && pa.data_y_max > 0) {
        auto zy = to_screen_y(pa, 0);
        painter.draw_line({pa.x, zy}, {pa.x + pa.w, zy}, axis_color, 0.5f);
    }
    ...
    for (auto const &s : series_) {
        for (auto const &p : s.points) {
            ...
            auto sy = to_screen_y(pa, p.y);
            painter.fill_circle({sx, sy}, s.point_radius,
                                Color::rgba(s.color.r, s.color.g, s.color.b, 0.7f));
        }
    }
    ...
    if (hover_ && hover_->series_idx < series_.size()) {
        ...
        if (hover_->point_idx < s.points.size()) {
            ...
            painter.fill_circle({sx, sy}, s.point_radius + 2, s.color);
            painter.draw_circle({sx, sy}, s.point_radius + 2, bg, 1.5f);

            auto const &dp = s.points[hover_->point_idx];

            auto tip = fmt::format("{}  x:{:.4g}  y:{:.4g}", dp.label.empty() ? s.name : dp.label,
                                   dp.x, dp.y);
    ...
    if (show_legend_ && !series_.empty()) {
        ...
        for (auto const &s : series_) {
            painter.fill_circle({lx + 4, ly}, 4, s.color);
            lx += 12;
...
bool ScatterPlot::handle_mouse(MouseEvent const &event) {
    auto pa = compute_plot_area();
    ...
    if (event.type == MouseEvent::Type::Move || event.type == MouseEvent::Type::Drag) {
        ...
        for (auto si = size_t{0}; si < series_.size(); si++) {
            for (auto pi = size_t{0}; pi < series_[si].points.size(); pi++) {
                auto sx = to_screen_x(pa, series_[si].points[pi].x);
                auto sy = to_screen_y(pa, series_[si].points[pi].y);
...
Size ScatterPlot::size_hint() const { return {400, 300}; }

CursorShape ScatterPlot::cursor() const {
...
nlohmann::json ScatterPlot::to_json() const {
