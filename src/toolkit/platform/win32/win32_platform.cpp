...
static char vk_to_base_char(WPARAM vk) {
    if (vk >= 'A' && vk <= 'Z') return static_cast<char>('a' + (vk - 'A'));
...
static int detect_click_count(Win32PlatformApplication::WindowData &data,
                              int button, int x, int y, DWORD time) {
    ...
    if (same_button && in_time && in_range) data.click_count++;
    else data.click_count = 1;
...
LRESULT CALLBACK tk_wnd_proc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    case WM_MBUTTONDOWN: {
        ...
        e.position = {static_cast<float>(mx) / scale,
    ...
    case WM_MOUSEWHEEL: {
        ...
        e.position = {static_cast<float>(pt.x) / scale,
    ...
    case WM_CHAR: {
        ...
        if (wc < 32) break;
        char utf8[8] = {};
        WideCharToMultiByte(CP_UTF8, 0, &wc, 1, utf8, sizeof(utf8) - 1,
                            nullptr, nullptr);
        KeyEvent ke;
        ke.type = KeyEvent::Type::Press;
        ke.text = utf8;
...
int Win32PlatformApplication::run() {
    ...
    while (GetMessageW(&msg, nullptr, 0, 0)) {
        if (msg.message == WM_TK_INVOKE && msg.hwnd == nullptr) {
            ...
            { std::lock_guard lock(posted_mutex); fns.swap(posted_fns); }
            for (auto &fn : fns) fn();
            continue;
        }
...
void Win32PlatformApplication::post_to_main_thread(std::function<void()> fn) {
    { std::lock_guard lock(posted_mutex); posted_fns.push_back(std::move(fn)); }
...
void Win32PlatformApplication::clipboard_set_text(std::string const &text) {
    ...
    MultiByteToWideChar(CP_UTF8, 0, text.data(),
                        static_cast<int>(text.size()), dest, wlen);
    dest[wlen] = L'\0';
    GlobalUnlock(hg);
...
Win32PlatformWindow::Win32PlatformWindow(Win32PlatformApplication *app,
                                         std::string_view title, Size size,
                                         Window *owner)
    ...
    RECT r = {0, 0, static_cast<LONG>(size.width * scale),
              static_cast<LONG>(size.height * scale)};
    AdjustWindowRectEx(&r, WS_OVERLAPPEDWINDOW, FALSE, 0);
    hwnd = CreateWindowExW(0, Win32PlatformApplication::kWindowClassName,
                           wtitle.c_str(), WS_OVERLAPPEDWINDOW,
...
int Win32PlatformWindow::start_timer(float interval_sec,
...
void Win32PlatformWindow::set_cursor(CursorShape shape) {
    HCURSOR hc;
    switch (shape) {
    case CursorShape::IBeam:      hc = ibeam_cursor; break;
    case CursorShape::Hand:       hc = hand_cursor; break;
    case CursorShape::NotAllowed: hc = not_allowed_cursor; break;
    default:                      hc = arrow_cursor; break;
    }
    auto it = app_->window_map.find(hwnd);
    if (it != app_->window_map.end()) it->second.current_cursor = hc;
    SetCursor(hc);
}

void Win32PlatformWindow::show_tooltip_window(std::string const &text,
                                              Point local_pos) {
    float scale = get_window_scale(hwnd);
    ...
    POINT pt = {static_cast<LONG>(local_pos.x * scale),
                static_cast<LONG>(local_pos.y * scale)};
    ...
    RECT work = mi.rcWork;
    if (sx + piw > work.right) sx = work.right - piw - 2;
    if (sx < work.left) sx = work.left + 2;
    if (sy < work.top) sy = pt.y + 20;
    DWORD ex_style = WS_EX_TOOLWINDOW | WS_EX_TOPMOST | WS_EX_NOACTIVATE |
                     WS_EX_LAYERED;
    if (!tooltip_hwnd) {
        tooltip_hwnd = CreateWindowExW(
            ex_style, Win32PlatformApplication::kTooltipClassName, L"",
            WS_POPUP, sx, sy, piw, pih, hwnd, nullptr, app_->hinstance,
            nullptr);
    } else {
    ...
    painter.draw_rounded_rect(r, style.border, style.corner_radius,
                              style.border_width);
...
void Win32PlatformWindow::hide_tooltip_window() {
    if (tooltip_hwnd) ShowWindow(tooltip_hwnd, SW_HIDE);
}
