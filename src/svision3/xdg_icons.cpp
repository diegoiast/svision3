...
IconLoader::IconLoader() : loader(detail::current_platform()->create_image_loader()) {
...
auto IconLoader::set_theme(std::string_view theme_name) -> void {
    ...
    if (t) {
        theme = std::make_unique<XDG::IconTheme>(std::move(*t));
    } else {
        theme.reset();
    }
}

auto IconLoader::theme_name() const -> std::string_view { return current_theme; }

auto IconLoader::load_theme(std::string_view theme_name) -> std::optional<XDG::IconTheme> {
...
auto IconLoader::parse_index_theme(std::string_view path) -> std::optional<XDG::IconTheme> {
...
auto IconLoader::find_icon_path(std::string_view icon_name, int size, std::string_view context)
    -> std::optional<std::string> {
    if (!theme) {
        return std::nullopt;
    }

    auto search_theme = [&](const XDG::IconTheme &t) -> std::optional<std::string> {
...
auto IconLoader::load(std::string_view icon_name, int size, std::string_view context) -> Icon {
