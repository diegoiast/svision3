...
#include "svision3/painters/cairo_shaper.hpp"
...
class WaylandPlatformWindow : public PlatformWindow {
  public:
    ...
    // False until the first show() call, and again between hide()/show().
    // Starting false (rather than true) matches X11 and Win32, where
    // CreateWindow/XCreateWindow also don't make a window visible on their
    // own -- an app must call show() explicitly on every backend now,
    // instead of Wayland windows appearing on their own as soon as the
    // compositor answers the initial configure.
    //
    // An unmapped surface never fires the frame callback do_paint() requests
    // (frame callbacks schedule the next *presented* frame, and an unmapped
    // surface isn't being presented), so painting while unmapped -- which
    // happens routinely from background timers calling request_redraw() --
    // would leave frame_cb stuck forever, blocking every future repaint
    // including the one show() needs. The main loop's redraw check gates on
    // this so no paint (and thus no frame_cb request) happens until show()
    // sets it.
    bool mapped = false;
