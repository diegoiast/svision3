...
class TextEditCommand : public UndoCommand {
  public:
    ...
    TextEditCommand(std::vector<std::string> *lines, std::vector<std::string> old_lines,
                    std::vector<std::string> new_lines, std::function<void()> on_change)
        : lines_(lines), old_lines_(std::move(old_lines)), new_lines_(std::move(new_lines)),
