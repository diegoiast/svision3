...
class StackedBarChart : public Widget {
  public:
    StackedBarChart();
    ...
    void set_categories(std::vector<StackedBarCategory> categories);
    void add_series(StackedBarSeries series);
    void clear();
    ...
    void set_normalized(bool v) { normalized_ = v; }

    CursorShape cursor() const override;
    nlohmann::json to_json() const override;

    std::function<void(size_t series_idx, size_t category_idx)> on_hover;

  private:
    mutable ChartInteraction interaction_;

    std::vector<StackedBarCategory> categories_;
    std::vector<StackedBarSeries> series_;
    std::string title_;
    std::string x_label_;
    std::string y_label_;
