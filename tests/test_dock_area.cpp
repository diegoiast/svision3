...
TEST_CASE("Window-routed click on a collapsed dock's tab bar un-collapses it", "[dockarea]") {
    ...
    dock->set_center(std::make_shared<Label>("center"));
    dock->add_dock(DockPosition::South, "Console", std::make_shared<Button>("Focusable content"));
    auto south = dock->dock_tab_widget(DockPosition::South).lock();
    REQUIRE(south);
